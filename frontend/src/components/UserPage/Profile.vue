...
<template>
    <section class="flex gap-10 w-full justify-center flex-wrap ">
        ...
        <div class="flex flex-col gap-2 w-full md:w-[300px]">
            ...
            <button class="text-red-600 text-sm hover:underline" @click="isOpenConfirmDelete = true">Delete Account</button>
        </div>
        ...
        <form class="flex flex-col gap-2 w-full md:w-[300px] " @submit.prevent="changePassword">
            ...
            <FormInput v-model="password.current" id="currentPassword" label="Current Password" type="password" placeholder="Enter Current Password" />
            <FormInput v-model="password.new"  id="password" label="New Password" type="password" placeholder="Enter New Password" />
            <FormInput v-model="password.confirm" id="confirmPassword" label="Confirm Password" type="password" placeholder="Confirm Password" />
        ...
        <div class="fixed  w-full  top-0 left-0 z-30 pointer-events-none">
            ...
            <Transition>
                ...
                <p class="text-sm">
                    Deleting your account is permanent and cannot be undone. This will also delete all your posts and personal data associated with your account.
                </p>
                <div class="flex flex-col md:flex-row w-full  gap-2 ">
                    <ButtonLink :isLink="false" type="button" content="Proceed" :fun="()=>{deleteAccount()}" />
                    <ButtonLink :isLink="false" type="button" content="Cancel " :fun="()=>{isOpenConfirmDelete = false}"/>
                </div>
                </div>
            </Transition>
        </div>
