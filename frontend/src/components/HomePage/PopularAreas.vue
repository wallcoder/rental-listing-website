    ...
    <section>
        ...
        <div v-else class="flex flex-col gap-4 py-10 border-b-2 border-gray-200 ">
            ...
            <div class="w-full grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 md:px-18 lg:px-24 m-auto">
                <RouterLink :to="`/browse?street=&locality=&city=${n?.city}&state=Mizoram&pincode=&country=India`" v-for="n in area" :key="n" class=" rounded-lg overflow-hidden relative group">
                    <img src="@/assets/mizoram.jpg" alt=""
                        class="object-cover rounded-lg brightness-[0.8] group-hover:brightness-[0.9] group-hover:scale-110 transition-all duration-500 ">
